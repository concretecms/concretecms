...
<form method="post" id="ccm-file-pick-set-form" action="<?=REL_DIR_FILES_TOOLS_REQUIRED?>/files/pick_set/">
<?=$form->hidden('task', 'pick_set')?>
<h1><?=t('File Sets')?></h1>

<div class="ccm-file-set-pick-cb">
	<?=$form->select('fsID', $sets, isset($_GET['oldFSID']) ? $_GET['oldFSID'] : false)?>
</div>
