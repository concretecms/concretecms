...
class ConcreteRequest {
	...
	private static function parsePathFromRequest($var) {
		...
		$path = trim($path, '/');
		if ($path != '' && $path != DISPATCHER_FILENAME) {
			return $path;
		} else {
			return false;
		}
	...
	private function parse() {
		...
		// tools
		if (preg_match("/^tools\/required\/(.[^\.]*).php/i", $path, $matches)) {
			$this->filename = $matches[1] . '.php';
			$this->includeType = 'CONCRETE_TOOL';
			return;
		}

		if (preg_match("/^tools\/(.[^\.\/]*).php/i", $path, $matches)) {
			$this->filename = $matches[1] . '.php';
			$this->includeType = 'TOOL';
			return;
		}

		if (preg_match("/^tools\/blocks\/(.[^\/]*)\/(.[^\.]*).php/i", $path, $matches)) {
