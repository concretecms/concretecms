<?
	class UserInfo extends Object {
		...
		private function get($where, $var, $userPermissionsArray = null) {
			$db = Loader::db();
			$q = "select Users.uID, Users.uLastLogin, Users.uPreviousLogin, Users.uNumLogins, Users.uDateAdded, Users.uIsActive, Users.uLastOnline, Users.uHasAvatar, Users.uName, Users.uEmail, Users.uPassword from Users " . $where;
		...
		public static function add($data) {
			...
			$v = array($data['uName'], $data['uEmail'], User::encryptPassword($data['uPassword']), $uIsActive, $uDateAdded);
			$r = $db->prepare("insert into Users (uName, uEmail, uPassword, uIsActive, uDateAdded) values (?, ?, ?, ?, ?)");
			$res = $db->execute($r, $v);
			if ($res) {
				$newUID = $db->Insert_ID();
		...
		public function register($data) {
			...
			if (is_object($ui)) {
				Statistics::record($ui->getUserID(), null, null, 'REGISTER');
			}
			return $ui;
		}

		function getUserActivationHash($uEmail) {
			// useful for user email validation
			if (defined("VALIDATE_USER_EMAIL_SALT")) {
				$hash = md5(VALIDATE_USER_EMAIL_SALT . ':' . $uEmail);
				return $hash;
			}
		}
		
		function validateUserEmailAddress($uEmail, $uHash) {
			if ($uHash == md5(VALIDATE_USER_EMAIL_SALT . ':' . $uEmail)) {
				$db = Loader::db();
				$v = array($uEmail);
				$db->query("update Users set uIsActive = 1 where uEmail = ?", $v);
				$uID = $db->getOne("select uID from Users where uEmail = ?", $v);
				if ($uID > 0) {
