...
class UserAttributeKey extends Object {
	...
	function validateSubmittedRequest() {
		...
		foreach($list as $l) {
			if (!trim($_REQUEST['uak_' . $l->getKeyID()])) {
				$return[] = $l->getKeyName();
			}
		}		
		return $return;
	...
	function outputHTML($uID = null) {
		$ir = (isset($_REQUEST['uak_' . $this->ukID]));
		$text = "";
		$db = Loader::db();
		if ($uID) {
		...
		if ($ir) {
			$text = htmlentities($_REQUEST['uak_' . $this->ukID]);
		}
		switch($this->ukType) {
			case "TEXT":
				return '<input type="text" name="uak_' . $this->ukID . '" id="uak_' . $this->ukID . '" value="' . $text . '" class="uak-text" />';
				break;
			case "TEXTAREA":
				return '<textarea id="uak_' . $this->ukID . '" name="uak_' . $this->ukID . '" class="uak-textarea">' . $text . '</textarea>';
				break;
			case "BOOLEAN":
				$checked = ($text == 1) ? 'checked' : '';
				return '<input type="checkbox" name="uak_' . $this->ukID . '" id="uak_' . $this->ukID . '" value="1" ' . $checked . ' />';
				break;
			case "SELECT":
				$r = '<select name="uak_' . $this->ukID . '" id="uak_' . $this->ukID . '" class="uak-select">';
			...
			case "RADIO":
				...
				foreach($arr as $v) {
					...
					$r .= '<div class="uak-radio"><input type="radio" name="uak_' . $this->ukID . '" id="uak_' . $this->ukID . '_' . $i . '" value="' . $v . '" ' . $checked . ' /> ' . $v . '</div>';			
					$i++;
				}
	...
	function outputSearchHTML($uID = null) {
		...
		switch($this->ukType) {
			case "TEXT":
				return '<input type="text" name="uak_' . $this->ukID . '" value="' . $text . '" class="uak-text" />';
				break;
			case "TEXTAREA":
				return '<input type="text" name="uak_' . $this->ukID . '" value="' . $text . '" class="uak-text" />';
				break;
			case "BOOLEAN":
				$checked = ($text == 1) ? 'checked' : '';
				return '<input type="checkbox" name="uak_' . $this->ukID . '" value="1" ' . $checked . ' />';
				break;
			case "SELECT":
				$r = '<select name="uak_' . $this->ukID . '" class="uak-select">';
				$r .= '<option value="">N/A</option>';
				$arr = preg_split("/\r|\n/", trim($this->getKeyValues()), -1, PREG_SPLIT_NO_EMPTY);
				foreach($arr as $v) {
					$selected = ($text == $v) ? 'selected' : '';
			...
			case "RADIO":
				...
				foreach($arr as $v) {
					$checked = ($text == $v) ? 'checked' : '';
					$r .= '<div class="uak-radio"><input type="radio" name="uak_' . $this->ukID . '" value="' . $v . '" ' . $checked . ' /> ' . $v . '</div>';			
				}
				return $r;
				break;
