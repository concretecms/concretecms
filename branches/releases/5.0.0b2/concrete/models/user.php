	...
	class User extends Object {
		...
		function User() {
			...
			if ($args[1]) {
				...
				$q = "select uID, uName, uIsActive from Users where uName = '$username' and uPassword = '$password'";
				...
				$this->superUser = ($_SESSION['uName'] == USER_SUPER) ? true : false;
			}
		...
		function recordView($c) {
			...
			$v = array($cID, $uID);
			$db->query("insert delayed into PageStatistics (cID, uID) values (?, ?)", $v);
		}
