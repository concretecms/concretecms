		...
		try {
			...
			if ($val->test()) {
				...
				if (!is_writable(DIR_BASE . '/config')) {
					throw new Exception('Your configuration directory web/config/ does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_UPLOADED)) {
					throw new Exception('Your files directory ' . REL_DIR_FILES_UPLOADED . ' does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_UPLOADED_THUMBNAILS)) {
					throw new Exception('Your files directory ' . REL_DIR_FILES_UPLOADED_THUMBNAILS . ' does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_UPLOADED_ONSTATES)) {
					throw new Exception('Your files directory ' . REL_DIR_FILES_UPLOADED_ONSTATES . ' does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_TRASH)) {
					throw new Exception('Your files directory files/trash does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_CACHE)) {
					throw new Exception('Your files directory files/cache does not appear to be writable by the web server.');
				}

				if (!is_writable(DIR_FILES_AVATARS)) {
					throw new Exception('Your files directory files/avatars does not appear to be writable by the web server.');
				}
				...
				if (!$db) {
					throw new Exception('Unable to connect to a database using those credentials.');
				}
				
				$num = $db->GetCol("show tables");
				if (count($num) > 0) {
					throw new Exception('There are already ' . count($num) . ' tables in this database. Concrete must be installed in an empty database.');
				}
