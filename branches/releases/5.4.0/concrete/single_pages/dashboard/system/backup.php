...
<?php echo $ifHelper->button('Delete Backup',$this->action('delete_backup'),'right');?></span>
</div> 
...
<?php echo $ifHelper->button_js(t('Cancel'),"$.fn.dialog.close(0)", 'left');?>
<span class="confirmActionBtn">
<?php echo $ifHelper->button('Restore Backup',$this->action('restore_backup'),'right');?></span>
</div> 
...
<tr>
	<td style="padding-right: 20px">
		<?php echo  $ifHelper->button(t("Run Backup"),$this->action('run_backup'),"left",null,array('id' => 'executeBackup'))?>
	</td>
	<td>
	<?php  if ($crypt->isAvailable()) { ?>
		<input type="checkbox" name="useEncryption" id="useEncryption" value="1" checked />
		<?php echo t('Use Encryption? (Note: this is <strong>strongly</strongly> recommended.)')?>
	<?php  } else { ?>
	...
	<?php echo t('This will create a database export file and store it on your server. It tries to encrypt the file if possible, 
