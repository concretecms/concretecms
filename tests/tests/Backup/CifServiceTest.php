...
class CifServiceTest extends TestCase
{
    ...
    public function testGetBooleanValues(string $xml, bool $expected, string $childName = '', bool $isDefaultFallback = false): void
    {
        ...
        $actual = CifService::getBool($element);
        $this->assertSame($expected, $actual);
        if ($isDefaultFallback) {
            $actual = CifService::getBool($element, true);
    ...
    {
        $element = $this->parseXml($xml);
        $actual = CifService::getBool($element[$attributeName]);
        $this->assertSame($expected, $actual);
        if ($isDefaultFallback) {
            $actual = CifService::getBool($element[$attributeName], true);
            $this->assertSame(true, $actual);
        }
    }
