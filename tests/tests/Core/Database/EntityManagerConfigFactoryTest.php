...
{     
     ...
     * so settings can be saved directly to tests/config/database.php
     * instead of tests/config/generated_overrides
     *
     * Used to store the orm metadata of packages
     *
     * @return \Concrete\Core\Config\Repository\Repository
     */
    protected function getTestFileConfigORMMetadata()
    {
        $defaultEnv      = \Config::getEnvironment();
        $fileSystem      = new \Illuminate\Filesystem\Filesystem();
        $fileLoader      = new \Concrete\Core\Config\FileLoader($fileSystem);
        $directFileSaver = new \Concrete\Core\Config\DirectTestFileSaver($fileSystem);
