		...
		<form id="popupLoginForm" method="post" action="<?=$loginFormSubmitURL ?>" onSubmit="return ccmPopupLogin.login(this);">
			...
			<div id="loginFormColums" style="position:relative; left:0px; top:0px; width:100%; height:auto;"> 
				...
				<div id="loginFormRight" style="margin-top:16px; width:33%; text-align:right; float:right; margin-right:20px; height:auto; margin-top:80px; "> 
					 ...
					 <?= t("Don't have an account on <Br />the concrete5 marketplace?"); ?>
					 </div>
			...
			<a onclick="$('#ccm-popup-login-submit').click()" class="ccm-button-right"><span><em class=""><?=t('Sign In')?> &gt;</em></span></a>	
			...
			<table>
				<tr>
					...
					<td style="padding-left:12px">  
						<div class="ccm-buttons"> 
							<a onclick="$('#ccm-popup-forgot-pass-submit').click()" class="ccm-button-right"><span><em><?=t('Reset and Email Password')  ?> &gt;</em></span></a>	
	...
	<div id="ccm-popupRegisterWrap" class="ccm-form" style="display:none">
		...
		<form id="popupRegisterForm" method="post" action="<?=$registerFormSubmitURL ?>" onSubmit="return ccmPopupLogin.submitRegister(this);">
			...
			<div id="popupRegisterColRight" style="float:right; width:48%;">
				<?=$form->checkbox('uTermsConditions', 'terms')?>	<?=t('I agree to the <a href="%s" target="_blank">terms and conditions</a>.','http://www.concrete5.org/help/legal') ?>
			...
			<div class="ccm-buttons" style="width:40%; float:right"> 
				<a onclick="$('#ccm-popup-register-submit').click()" class="ccm-button-right"><span><em><?=t('Register') ?>  &gt;</em></span></a>	
				<input type="submit" name="submit" value="submit" style="display: none" id="ccm-popup-register-submit" />
			</div>	
			
			<div style="margin-top:16px; width:40%; padding-top:16px; "><a onClick="ccmPopupLogin.toggleRegister()"><?=t('&laquo; Return to login')?></a></div>
