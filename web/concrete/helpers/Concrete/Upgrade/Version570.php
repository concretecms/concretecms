...
namespace Concrete\Helper\Concrete\Upgrade;
class Version570 {
	...
	public function run() {
		...
		if ($sp->isError()) {
			$sp = SinglePage::addSinglePage('/dashboard/users/points');
		...
		if ($sp->isError()) {
			$sp = SinglePage::addSinglePage('/dashboard/users/points/assign');
			$sp->update(array('cName'=>t('Assign Points')));
		}

		$sp = Page::getByPath('/dashboard/users/points/actions');
		if ($sp->isError()) {
			$sp = SinglePage::addSinglePage('/dashboard/users/points/actions');
		}
