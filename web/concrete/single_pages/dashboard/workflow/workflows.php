...
<form method="post"  action="<?=$view->action('save_workflow_details')?>" method="post" class="form-horizontal">
...
<? } else if ($this->controller->getTask() == 'add' || $this->controller->getTask() == 'submit_add') { ?>
	...
	<form method="post" class="form-horizontal" action="<?=$view->action('submit_add')?>" id="ccm-attribute-type-form">
...
<? } else { ?>
	...
	<a href="<?=View::url('/dashboard/workflow/workflows', 'add')?>" style="float: right" class="btn btn-primary"><?=t("Add Workflow")?></a>
	...
	<div class="ccm-workflow">
		<a href="<?=$view->url('/dashboard/workflow/workflows', 'view_detail', $workflow->getWorkflowID())?>"><?=$workflow->getWorkflowName()?></a>
	</div>
	<? } ?>
