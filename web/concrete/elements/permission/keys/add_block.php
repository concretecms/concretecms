...
<div class="form-group">
	...
	<div class="inputs-list" <?php if ($assignment->getBlockTypesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($blockTypes as $bt) { ?>
			<div class="checkbox"><label><input type="checkbox" name="btIDInclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$bt->getBlockTypeID()?>" <?php if (in_array($bt->getBlockTypeID(), $assignment->getBlockTypesAllowedArray())) { ?> checked="checked" <?php } ?> /> <?=t($bt->getBlockTypeName())?></label></div>
		<?php } ?>
	</div>
</div>
...
<div class="form-group">
	...
	<div class="inputs-list" <?php if ($assignment->getBlockTypesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($blockTypes as $bt) { ?>
        <div class="checkbox"><label><input type="checkbox" name="btIDExclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$bt->getBlockTypeID()?>" <?php if (in_array($bt->getBlockTypeID(), $assignment->getBlockTypesAllowedArray())) { ?> checked="checked" <?php } ?> /> <span><?=t($bt->getBlockTypeName())?></span></label></div>
		<?php } ?>
	</div>
</div>
