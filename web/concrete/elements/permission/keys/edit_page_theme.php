...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <? if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<? } ?>>
		<? foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="pThemeIDInclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$ct->getThemeID()?>" <? if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'A') { ?> checked="checked" <? } ?> /> <span><?=$ct->getThemeName()?></span></label></li>
		<? } ?>
	</ul>
	</div>
</div>
...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <? if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<? } ?>>
		<? foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="pThemeIDExclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$ct->getThemeID()?>" <? if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'N') { ?> checked="checked" <? } ?> /> <span><?=$ct->getThemeName()?></span></label></li>
		<? } ?>
	</ul>
	</div>
</div>
