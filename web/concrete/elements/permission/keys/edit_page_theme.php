...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <?php if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="pThemeIDInclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$ct->getThemeID()?>" <?php if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'A') { ?> checked="checked" <?php } ?> /> <span><?=$ct->getThemeDisplayName()?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
...
<div class="clearfix">
	...
	<ul class="theme-list inputs-list" <?php if ($assignment->getThemesAllowedPermission() != 'C') { ?>style="display: none"<?php } ?>>
		<?php foreach($themes as $ct) { ?>
			<li><label><input type="checkbox" name="pThemeIDExclude[<?=$entity->getAccessEntityID()?>][]" value="<?=$ct->getThemeID()?>" <?php if (in_array($ct->getThemeID(), $assignment->getThemesAllowedArray()) || $assignment->getThemesAllowedPermission() == 'N') { ?> checked="checked" <?php } ?> /> <span><?=$ct->getThemeDisplayName()?></span></label></li>
		<?php } ?>
	</ul>
	</div>
</div>
