...
ccm_addAccessEntity = function(peID, pdID, accessType) {
	...
	$.get('<?=$pk->getPermissionKeyToolsURL("add_access_entity")?>&pdID=' + pdID + '&accessType=' + accessType + '&peID=' + peID, function(r) { 
		$.get('<?=REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/dialogs/miscellaneous?message=entity_added&pkID=<?=$pk->getPermissionKeyID()?>', function(r) { 
...
ccm_deleteAccessEntityAssignment = function(peID) {
	...
	$.get('<?=$pk->getPermissionKeyToolsURL("remove_access_entity")?>&peID=' + peID, function() { 
		$.get('<?=REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/dialogs/miscellaneous?message=entity_removed&pkID=<?=$pk->getPermissionKeyID()?>', function(r) { 
			jQuery.fn.dialog.replaceTop(r);
			jQuery.fn.dialog.hideLoader();
		});
	});
...
ccm_submitPermissionCustomOptionsForm = function() {
	...
	$("#ccm-permissions-custom-options-form").ajaxSubmit(function(r) {
		$.get('<?=REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/dialogs/miscellaneous?message=custom_options_saved&pkID=<?=$pk->getPermissionKeyID()?>', function(r) { 
			jQuery.fn.dialog.replaceTop(r);
			jQuery.fn.dialog.hideLoader();
		});
	});
...
ccm_submitPermissionWorkflowForm = function() {
	...
	$("#ccm-permissions-workflow-form").ajaxSubmit(function(r) {
		$.get('<?=REL_DIR_FILES_TOOLS_REQUIRED?>/permissions/dialogs/miscellaneous?message=workflows_saved&pkID=<?=$pk->getPermissionKeyID()?>', function(r) { 
			jQuery.fn.dialog.replaceTop(r);
			jQuery.fn.dialog.hideLoader();
		});
	});
