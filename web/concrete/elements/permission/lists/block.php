...
ccm_permissionLaunchDialog = function(link) {
	...
	jQuery.fn.dialog.open({
		...
		href: '<?=REL_DIR_FILES_TOOLS_REQUIRED?>/edit_block_popup?duplicate=' + dupe + '&bID=<?=$b->getBlockID()?>&arHandle=<?=$b->getAreaHandle()?>&cvID=<?=$c->getVersionID()?>&bID=<?=$b->getBlockID()?>&cID=<?=$c->getCollectionID()?>&btask=set_advanced_permissions&pkID=' + $(link).attr('data-pkID') + '&paID=' + $(link).attr('data-paID'),
		modal: false,
		width: 500,
		height: 380
	});		
