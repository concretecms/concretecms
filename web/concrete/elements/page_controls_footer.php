...
if (isset($cp) && $canViewToolbar && (!$dh->inDashboard())) {
    ...
    if (!$c->isEditMode()) {
        echo Core::make('helper/concrete/ui/help')->displayHelpDialogLauncher();
    }
    ...
    ?>
                <li>
                    <i class="fa fa-sign-out mobile-leading-icon"></i><a href="<?= URL::to('/login', 'logout', $valt->generate('logout'));
    ?>"><?= t('Sign Out');
    ?></a>
                </li>
            </ul>
