	...
	<? if (count($blockTypes) > 0) { 
		foreach($blockTypes as $bt) { 
			...
			<li class="ccm-block-type ccm-block-type-available">
				...
				<? } else if ($bt->supportsInlineEditing()) { ?>
					<a style="background-image: url(<?=$btIcon?>)" href="javascript:void(0)" onclick="ccmBlockTypeResetKeys(); ccm_loadInlineEditorAdd(<?=$c->getCollectionID()?>, '<?=htmlspecialchars($a->getAreaHandle())?>', <?=$a->getAreaID()?>, <?=$bt->getBlockTypeID()?>)" class="ccm-block-type-inner"><?=$bt->getBlockTypeName()?></a>
				<? } else { ?>
