...
<section>
	...
	<nav>
	<span><?php if (!in_array($tab, array('favorites'))) { ?><?=t('Dashboard')?><?php } else { ?><?=t('Favorites')?><?php } ?></span>
	<ul class="ccm-panel-header-accordion-dropdown">
		<li><a data-panel-accordion-tab="dashboard" <?php if (!in_array($tab, array('favorites'))) { ?>data-panel-accordion-tab-selected="true" <?php } ?>><?=t('Dashboard')?></a></li>
		<li><a data-panel-accordion-tab="favorites" <?php if ($tab == 'favorites') { ?>data-panel-accordion-tab-selected="true" <?php } ?>><?=t('Favorites')?></a></li>
	</ul>
	</nav>
	</div>
		...
		<?php if ($tab == 'favorites') { ?>
			...
			<?php foreach($nav as $cc) {
				...
				if ($cp->canViewPage()) { ?>
					<li><a href="<?=Loader::helper('navigation')->getLinkToCollection($cc)?>" <?php if ($active) { ?>class="ccm-panel-dashboard-nav-active"<?php } ?>><?=t($cc->getCollectionName())?></a></li>
				<?php } ?>
			<?php } ?>
			</menu>
		<?php } else { ?>
