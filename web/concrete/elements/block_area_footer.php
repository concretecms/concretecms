...
<div class="ccm-area-footer-handle" data-area-menu-handle="<?=$a->getAreaID()?>" id="area-menu-footer-<?=$a->getAreaID()?>"><span><i class="glyphicon glyphicon-share-alt"></i> <?=$a->getAreaDisplayName()?></span></div>
		...
		if ($axp->canAddBlockToArea($bt)) { 
			...
			if (is_object($bx) && !$bx->isError()) { ?>
				...
				<? if ($pk->validate()) { 
					...
					<li><a class="dialog-launch" href="<?=URL::to('/system/dialogs/area/layout/presets', $arLayout->getAreaLayoutID(), Loader::helper('validation/token')->generate('layout_presets'))?>" dialog-width="360" dialog-height="240" dialog-modal="true"><?=t("Save Layout as Preset")?></a></li>
				<? } ?>
			<? } ?>
		<? }
	} ?>
