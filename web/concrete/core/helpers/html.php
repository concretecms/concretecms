...
class Concrete5_Helper_Html {
	...
	public function css($file, $pkgHandle = null) {
		...
		if (substr($file, 0, 4) == 'http' || substr($file, 0, 2) == '//') {
			...
			if ($v instanceof PathRequestView && $v->getThemeDirectory() != '' && file_exists($v->getThemeDirectory() . '/' . $file)) {
	...
	public function image($src, $width = false, $height = false, $attribs = null) {
		...
		if ($width == false && $height == false && (!isset($image['scheme']))) {
			...
			$v = View::getInstance();
			if ($v instanceof PathRequestView && $v->getThemeDirectory() != '' && file_exists($v->getThemeDirectory() . '/' . DIRNAME_IMAGES . '/' . $src)) {
