...
<ul id="ccm-layouts-toolbar" class="ccm-inline-toolbar ccm-ui">
		...
		<select name="gridType" id="gridType" style="width: auto !important">
			...
			<optgroup label="<?=t('Presets')?>">
			  	<?php foreach($presets as $pr) { ?>
				    <option value="<?=$pr->getIdentifier()?>" <?php if (is_object($selectedPreset) && $selectedPreset->getIdentifier() == $pr->getIdentifier()) { ?>selected<?php } ?>><?=$pr->getName()?></option>
				<?php } ?>
			</optgroup>
			<?php } ?>
		</select>
	...
	<li data-grid-form-view="themegrid">
		...
		<input type="text" name="themeGridColumns" id="themeGridColumns" style="width: 40px" <?php if ($controller->getTask() == 'add') {?>  data-input="number" data-minimum="<?=$minColumns?>" data-maximum="<?=$themeGridMaxColumns?>" <?php } ?> value="<?=$columnsNum?>" />
	...
	<li data-grid-form-view="custom" class="ccm-sub-toolbar-text-cell">
		...
		<input type="text" name="columns" id="columns" style="width: 40px" <?php if ($controller->getTask() == 'add') {?> data-input="number" data-minimum="<?=$minColumns?>" data-maximum="<?=$maxColumns?>" <?php } ?> value="<?=$columnsNum?>" />
		<?php if ($controller->getTask() == 'edit') {
			// we need this to actually go through the form in edit mode, for layout presets to be saveable in edit mode. ?>
	...
	<li class="ccm-inline-toolbar-button ccm-inline-toolbar-button-save">
	  <button class="btn btn-primary" type="button" id="ccm-layouts-save-button"><?php if ($controller->getTask() == 'add') { ?><?=t('Add Layout')?><?php } else { ?><?=t('Update Layout')?><?php } ?></button>
	</li>
</ul>
	...
	$('#ccm-layouts-edit-mode').concreteLayout({
		...
		'gridColumnClasses': [
			...
			<?php for ($i = 0; $i < count($classes); $i++) {
				$class = $classes[$i];?>
				'<?=$class?>' <?php if (($i + 1) < count($classes)) { ?>, <?php } ?>
