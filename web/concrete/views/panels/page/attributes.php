	...
	<?php foreach ($attributes as $set) { ?>
		<div class="ccm-menu-page-attributes-set">
			...
			<?php foreach ($set->attributes as $key) { ?>
				<li><a data-attribute-key="<?=$key->getAttributeKeyID()?>" <?php if (in_array($key->getAttributeKeyID(), $selectedAttributeIDs)) { ?>class="ccm-menu-page-attribute-selected" <?php } ?> href="#"><?=$key->getAttributeKeyName()?></a></li>
			<?php } ?>
			</ul>
		</div>
	<?php } ?>
