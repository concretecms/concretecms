	...
	public function update_set() {
		if (Loader::helper('validation/token')->validate('update_set')) { 
			...
			if (!$this->error->has()) {
				...
				$this->redirect('dashboard/settings/attribute_sets', $as->getAttributeSetKeyCategoryID(), 'set_updated');
			}
	...
	public function update_set_attributes() {
		if (Loader::helper('validation/token')->validate('update_set_attributes')) { 
			...
			if (!$this->error->has()) {
				...
				$this->redirect('dashboard/settings/attribute_sets', $cat->getAttributeKeyCategoryID(), 'set_updated');
			}	
	...
	public function delete_set() {
		if (Loader::helper('validation/token')->validate('delete_set')) { 
			...
			if (!$this->error->has()) {
				...
				$as->delete();
				$this->redirect('dashboard/settings/attribute_sets', $categoryID, 'set_deleted');
			}			
		} else {
			$this->error->add(Loader::helper('validation/token')->getErrorMessage());
		}
