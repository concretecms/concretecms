...
class DashboardSystemDeveloperLoggingController extends DashboardBaseController{
	/**
	* Dasboard page view
	*
		...
		if($this->token->validate('update_logging')){
			if($this->isPost()){
				...
				if($intLogErrorsCurrent == $intLogErrorsPost && $intLogEmailsCurrent == $intLogEmailsPost){
					$this->redirect('/dashboard/system/developer/logging');
				}
								
				Config::save('ENABLE_LOG_ERRORS', $intLogErrorsPost);
				Config::save('ENABLE_LOG_EMAILS', $intLogEmailsPost);
				
				$this->redirect('/dashboard/system/developer/logging', 'logging_saved');
			}
		}else{
