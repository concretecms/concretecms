...
class DashboardSystemMaintenanceModeController extends DashboardBaseController {
		...
		if ($this->isPost()) {
			if ($this->token->validate("update_maintenance")) {
				...
				if ($mode == 1) { 
					...
					$this->redirect('/dashboard/system/maintenance/mode','saved',"enabled");
					exit;
				} else {
					Config::save('SITE_MAINTENANCE_MODE', 0);
					$this->redirect('/dashboard/system/maintenance/mode','saved',"disabled");
					exit;
				}
			} else {
