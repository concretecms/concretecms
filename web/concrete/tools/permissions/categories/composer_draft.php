...
if ($canEditComposerPermissions) {
	...
	if ($_REQUEST['task'] == 'revert_to_global_composer_permissions' && Loader::helper("validation/token")->validate('revert_to_global_composer_permissions')) {
		$draft->resetComposerDraftPermissions();		
	}

	if ($_REQUEST['task'] == 'override_global_composer_permissions' && Loader::helper("validation/token")->validate('override_global_composer_permissions')) {
		$draft->doOverrideComposerPermissions(1);		
	}
