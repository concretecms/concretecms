    ...
    public function searchForm($list)
    {
        ...
        if ($address1) {
            $list->filterByAttribute(
                array('address1' => $this->attributeKey->getAttributeKeyHandle()),
                '%' . $address1 . '%',
                'like'
            );
        }
        if ($address2) {
            $list->filterByAttribute(
                array('address2' => $this->attributeKey->getAttributeKeyHandle()),
                '%' . $address2 . '%',
                'like'
            );
        }
        if ($city) {
            $list->filterByAttribute(
                array('city' => $this->attributeKey->getAttributeKeyHandle()),
                '%' . $city . '%',
                'like'
            );
        }
        if ($state_province) {
            $list->filterByAttribute(
                array('state_province' => $this->attributeKey->getAttributeKeyHandle()),
                $state_province
            );
        }
        if ($postal_code) {
            $list->filterByAttribute(
                array('postal_code' => $this->attributeKey->getAttributeKeyHandle()),
                '%' . $postal_code . '%',
                'like'
            );
        }
