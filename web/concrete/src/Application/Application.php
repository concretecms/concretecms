...
class Application extends Container
{
    ...
    public function shutdown($options = array())
    {
        ...
        if ($this->isInstalled()) {
            ...
            $logger = new Logger();
            $r = $this['request']->getInstance();

            if ($this['config']->get('concrete.log.queries.log') &&
                ...
                if ($logger->shouldLogQueries($r)) {
                    ...
                    if ($this['config']->get('concrete.log.queries.clear_on_reload')) {
    ...
    public function clearCaches()
    {
        ...
        $cacheDir = $this['config']->get('concrete.cache.directory');
        if (is_dir($cacheDir)) {
    ...
    {
        if ($this['config']->get('concrete.jobs.enable_scheduling')) {
            ...
            if ($c instanceof Page && !$c->isAdminArea()) {
                ...
                if (strlen($url)) {
                    ...
                    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, $this['config']->get('app.curl.verifyPeer'));
                    $res = curl_exec($ch);
                }
            }
        }
    ...
    {
        if ($this['config']->get('concrete.updates.enable_auto_update_core')) {
            $installed = $this['config']->get('concrete.version_installed');
            $core = $this['config']->get('concrete.version');
    ...
    public function setupPackages()
    {
        ...
        foreach($this->packages as $pkg) {
            // handle updates
            if ($this['config']->get('concrete.updates.enable_auto_update_packages')) {
        ...
        $this['config']->set('app.bootstrap.packages_loaded', true);
    ...
    public function setupFilesystem()
    {
        if (!is_dir($this['config']->get('concrete.cache.directory'))) {
            ...
            @touch($this['config']->get('concrete.cache.directory') . '/index.html', $this['config']->get('concrete.filesystem.permissions.file'));
        }
    }
    ...
    {
        if ($this['config']->get('concrete.seo.redirect_to_canonical_url') && $this['config']->get('concrete.seo.canonical_url')) {
            $url = UrlImmutable::createFromUrl($r->getUri());

            $canonical = UrlImmutable::createFromUrl($this['config']->get('concrete.seo.canonical_url'),
                (bool) $this['config']->get('concrete.seo.trailing_slash')
            );
            ...
            // URL
            if ($this['config']->get('concrete.seo.canonical_ssl_url')) {
                $ssl = UrlImmutable::createFromUrl($this['config']->get('concrete.seo.canonical_ssl_url'));
