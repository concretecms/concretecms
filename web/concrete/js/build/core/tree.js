		...
		getMenu: function() {
			return '<div data-tree-menu="<%=data.treeID%>" class="ccm-topic-menu ccm-popover-page-menu popover fade popover fade">' +
				...
				'<% if (data.canAddTopicCategoryTreeNode) { %>' +
                    ...
                    CCM_DISPATCHER_FILENAME + '/ccm/system/dialogs/tree/node/add/topic_category?treeNodeID=<%=data.key%>">' +
				...
				'<% if (data.canEditTreeNode && data.treeNodeTypeHandle == "topic_category") { %>' +
					'<li><a href="#" data-tree-action="edit-node" dialog-title="' + ccmi18n_topics.editCategory + '" data-tree-action-url="' + CCM_DISPATCHER_FILENAME + '/ccm/system/dialogs/tree/node/edit/topic_category?treeNodeID=<%=data.key%>">' + ccmi18n_topics.editCategory + '<\/a><\/li>' +
				'<% } %>' +
