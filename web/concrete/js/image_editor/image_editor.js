...
var ImageEditor = function(settings) {
    ...
    if (im.strictSize) {
        ...
        saveSize.both.keydown(function(e) {
            ...
            if (e.keyCode == 38) {
                ...
                $(this).val(Math.min(5e3, newval)).change();
            }
            if (e.keyCode == 40) {
                var newval = parseInt($(this).val()) - 1;
    ...
    im.save = function() {
        ...
        im.stage.setScale(1);
        var newx = im.image.getX() - im.dragger.getX(), newy = im.image.getY() - im.dragger.getY();
        im.image.setX(newx);
        im.image.setY(newy);
    ...
    im.adjustSavers = function() {
        log("Adjusting");
        var startx = Math.round(im.center.x - im.saveWidth / 2), posx = startx + im.saveWidth, starty = Math.round(im.center.y - im.saveHeight / 2), posy = starty + im.saveHeight;
        if (posy < starty) {
            var inter = posy;
            posy = starty;
            starty = inter;
