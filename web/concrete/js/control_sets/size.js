...
var controls = [
	new Control('topleft','nw-resize',function(c){
		c.control.setX(im.image.getX()-5);
		c.control.setY(im.image.getY()-5);
	},function(c){
		var newwidth = c.startSize.x + c.dragStart.x - c.control.getX();
		var newheight = c.startSize.y + c.dragStart.y - c.control.getY();
		im.image.setWidth(newwidth);
		im.image.setHeight(newheight);
		im.image.setX(c.control.getX()+5);
		im.image.setY(c.control.getY()+5);
		im.trigger('imagechange');
	}),
	new Control('bottomleft','sw-resize',function(c){
		c.control.setX(im.image.getX()-5);
		c.control.setY(im.image.getY()+im.image.getHeight()-5);
	},function(c){
		var newwidth = c.startSize.x + c.dragStart.x - c.control.getX();
		var newheight = c.startSize.y - (c.dragStart.y - c.control.getY());
		im.image.setWidth(newwidth);
	...
	new Control('topright','ne-resize',function(c){
		c.control.setX(im.image.getX()+im.image.getWidth()-5);
		c.control.setY(im.image.getY()-5);
	},function(c){
		var newwidth = c.startSize.x - (c.dragStart.x - c.control.getX());
		var newheight = c.startSize.y + c.dragStart.y - c.control.getY();
	...
	new Control('middleleft','w-resize',function(c){
		c.control.setX(im.image.getX()-5);
		c.control.setY(im.image.getY()+im.image.getHeight()/2-5);
	},function(c){
		var newwidth = c.startSize.x + c.dragStart.x - c.control.getX();
		im.image.setWidth(newwidth);
		im.image.setX(c.control.getX()+5);
	...
	new Control('middletop','n-resize',function(c){
		c.control.setX(im.image.getX()+im.image.getWidth()/2-5);
		c.control.setY(im.image.getY()-5);
	},function(c){
		var newheight = c.startSize.y + c.dragStart.y - c.control.getY();
		im.image.setHeight(newheight);
