                ...
                if ($p->canEditBlockDesign()) {
                    ...
                    if ($_POST['reset_css']) {
                        ...
                        if ($_POST['cspID'] > 0 && ($_POST['cspPresetAction'] == 'update_existing_preset')) {
                            ...
                            $csr = CustomStyleRule::add(
                                                  $_POST['css_id'],
                                                  $_POST['css_class_name'],
                                                  $_POST['css_custom'],
                                                  $_POST);
                            $b->setBlockCustomStyle($csr, $updateAll);
                        }
...
if (isset($_POST['processCollection']) && $_POST['processCollection'] && $valt->validate()) {
    if ($_POST['update_external']) {
        ...
        if ($parentP->canAddExternalLink()) {
            $ncID = $c->updateCollectionAliasExternal(
                      $_POST['cName'],
                      $_POST['cExternalLink'],
                      $_POST['cExternalLinkNewWindow']);
            ...
            if (is_object($nc)) {
                ...
                if ($_POST['rel'] == 'SITEMAP') {
                    ...
                    if ($_POST['mode'] == 'explore') {
                        header(
                            'Location: ' . BASE_URL . View::url('/dashboard/sitemap/explore', $c->getCollectionID()));
                        exit;
                    } else if ($_POST['mode'] == 'search') {
                        header(
                            'Location: ' . BASE_URL . DIR_REL . '/' . DISPATCHER_FILENAME . '?cID=' . $nc->getCollectionID() . '&mode=edit&ctask=check-out-first' . $step . $token);
                        exit;
                    } else {
                        Redirect::to('/dashboard/sitemap');
                    }
        ...
        if ($cp->canAddExternalLink()) {
            $ncID = $c->addCollectionAliasExternal(
                      $_POST['cName'],
                      $_POST['cExternalLink'],
                      $_POST['cExternalLinkNewWindow']);
            Redirect::to('/dashboard/sitemap');
