...
if (is_object($c)) {
	...
	if (!isset($pageTitle) || !$pageTitle) {
		...
		if (!$pageTitle) {
			...
			$seo->setSiteName(tc('SiteName', \Core::make('site')->getCurrentSite()->getSiteName()));
...
if ($modernIconFID) {
    ...
    if (is_object($f)) {
        ?>
        <meta name="msapplication-TileImage" content="<?php echo $f->getURL();
        ?>" /><?php
        echo "\n";
        if (strlen($modernIconBGColor)) {
            ?>
            <meta name="msapplication-TileColor" content="<?php echo $modernIconBGColor;
            ?>" /><?php
            echo "\n";
        }
    }
