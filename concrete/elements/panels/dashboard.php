...
<section>
  ...
  <span class="active-top-menu-title"><?php if (!in_array($tab, ['favorites'])) {
    ?><?=t('Dashboard'); ?><?php
} else {
        ?><?=t('Favorites'); ?><?php
    } ?></span>
	<ul class="ccm-panel-header-accordion-dropdown list-group">
		<li 
    class="list-group-item <?php if (!in_array($tab, ['favorites'])) {
        ?> active<?php
    }?>"
    ><a data-panel-accordion-tab="dashboard" <?php if (!in_array($tab, ['favorites'])) {
        ?>data-panel-accordion-tab-selected="true" <?php
    } ?>><?=t('Dashboard'); ?></a></li>
		<li class="list-group-item <?php if ($tab == 'favorites') {
        ?> active<?php
    }?>"><a data-panel-accordion-tab="favorites" <?php if ($tab == 'favorites') {
        ?>data-panel-accordion-tab-selected="true" <?php
    } ?>><?=t('Favorites'); ?></a></li>
	</ul>
	</nav>
        ...
        ?>
            ...
            for ($i = 0; $i < count($nav); ++$i) {
                ...
                if ($cp->canViewPage()) {
                    ?>
					<li <?php if ($active) {
                        ?>class="nav-selected"<?php
                    } ?>><a href="<?=Loader::helper('navigation')->getLinkToCollection($cc); ?>"><?=t($cc->getCollectionName()); ?></a></li>
