...
<section class="ccm-ui">
    ...
    ?>
                    <h3 class="font-weight-light"><?=t('Where will this page live on the site?')?></h3>
                <?php
} else {
    ?>
                    <h3 class="font-weight-light"><?=t('Where does this page live on the site?')?></h3>
                <?php
}
    ?>
    ...
    ?>
		...
		<h3 class="font-weight-light"><?=t('URLs to this Page')?></h3>

		<table class="mt-4 table table-striped ccm-page-panel-detail-location-paths">
			<thead>
			<tr>
				<th></th>
				<?php if (!$isHome) {
		...
		<p><small class="text-muted"><?=t('Note: Additional page paths are not versioned. They will be available immediately.')?></small></p>
...
<script type="text/template" id="breadcrumb-template">
	...
	<ol class="breadcrumb">
	  <li class="breadcrumb-item"><a href="<%=parentLink%>" target="_blank"><%=parentName%></a></li>
	  <li class="breadcrumb-item active"><?=$c->getCollectionName()?></li>
	</ol>
	<% } else { %>
		<div class="breadcrumb">
...
var renderBreadcrumb = _.template(
    $('script#breadcrumb-template').html()
);
