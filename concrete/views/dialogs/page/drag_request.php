...
<form id="<?= $formID ?>">
    ...
    if ($dragRequestData->getDestinationSibling() !== null) {
        ?>
    <input type="hidden" name="destSibling"
        value="<?= $dragRequestData->getDestinationSibling()->getCollectionID() ?>" />
    <?php
    }
    ?>
    ...
    <div class="form-group">
        <div class="form-check">
            <input type="radio" id="ctask1" name="ctask" class="form-check-input"
                value="<?= $dragRequestData::OPERATION_MOVE ?>" />
    ...
    <div class="form-group">
        <div class="form-check">
            <input type="radio" name="ctask" id="ctask2" class="form-check-input"
                value="<?= $dragRequestData::OPERATION_ALIAS ?>" />
            ...
            <div style="margin: 0 0 0 20px">
                <?php
                    if ($dragRequestData->canDo($dragRequestData::OPERATION_COPY)) {
                        ?>
                <div class="form-check">
                    <input type="radio" class="form-check-input" id="dtask1" name="dtask"
                        value="<?= $dragRequestData::OPERATION_COPY ?>" />
                ...
                <div class="form-check">
                    ...
                    <input type="radio" class="form-check-input" id="dtask2" name="dtask"
                        value="<?= $dragRequestData::OPERATION_COPYALL ?>" />
                ...
                <div class="form-check">
                    ...
                    <input type="radio" class="form-check-input" id="dtask3" name="dtask"
                        value="<?= $dragRequestData::OPERATION_COPYVERSION ?>" />
    ...
    <div class="dialog-buttons">
        <button onclick="$.fn.dialog.closeTop()" class="btn btn-secondary"><?= t('Cancel') ?></button>
        <button data-form-selector="#<?= $formID ?>"
            onclick="ConcreteSitemap.submitDragRequest($($(this).data('form-selector')))"
            class="ms-auto btn btn-primary"><?= t('Go') ?></button>
    </div>
</form>
...
$(document).ready(function() {
    ...
    function updateState() {
        ...
        muteInput('input[name="saveOldPagePath"]', ctask !==
            <?= json_encode($dragRequestData::OPERATION_MOVE) ?>);
    ...
    $form.find('input[name="saveOldPagePath"]').on('change click', function() {
        $form.find('input[name="ctask"][value="' +
                <?= json_encode($dragRequestData::OPERATION_MOVE) ?> + '"]')
            .prop('checked', true)
            .trigger('change');
    });
