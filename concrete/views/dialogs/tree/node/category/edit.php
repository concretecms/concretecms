...
<div class="ccm-ui">
	<form method="post" data-dialog-form="edit-topic-category-node" class="form-horizontal" action="<?= $controller->action('update_category_node'); ?>">
		<?= $validation_token->output('update_category_node'); ?>
		<input type="hidden" name="treeNodeID" value="<?= $node->getTreeNodeID(); ?>" />

		<div class="form-group">
			<?= $form->label('treeNodeCategoryName', t('Name')); ?>
			<?= $form->text('treeNodeCategoryName', $node->getTreeNodeName()); ?>
		</div>

		<div class="form-group">
			<?= $form->label('treeNodeStorageLocationID', t('Storage Location')); ?>
			<?= $form->select('treeNodeStorageLocationID', $locations, $node->getTreeNodeStorageLocationID()); ?>
		</div>
