...
class MiniSurvey
{
    ...
    public function addEditQuestion($values, $withOutput = 1)
    {
        ...
        if (strlen($values['question']) == 0 || strlen($values['inputType']) == 0  || $values['inputType'] == 'null') {
            ...
            $this->lastSavedqID = (int)$this->db->fetchColumn("SELECT MAX(qID) FROM btFormQuestions WHERE bID=0 AND msqID=?", array($values['msqID']));
    ...
    public function reorderQuestions($qsID = 0, $qIDs)
    {
        ...
        $positionNum = 0;
        foreach ($qIDs as $qID) {
            $vals = array($positionNum, (int)$qID, (int)$qsID);
