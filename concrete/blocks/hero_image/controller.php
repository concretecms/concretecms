...
class Controller extends BlockController implements ApiResourceValueInterface, ApiValueSchemaInterface, FileTrackableInterface, UsesFeatureInterface
{
    ...
    public function getApiValueSchema(): array
    {
        ...
        return [
            'type' => 'object',
            'properties' => [
                'image' => $schemaFactory->describeReference(ExportDeclarations::REFERENCE_FILE, [
                    'type' => ['string', 'integer', 'null'],
                    'description' => 'The image filling the block (0 for none).',
                ]),
                'height' => [
                    'type' => ['string', 'integer', 'null'],
                    'description' => 'The height of the block, as a percentage of the height of the window (0 to let its contents decide).',
                ],
                'icon' => [
                    'type' => ['string', 'null'],
                    'description' => 'The class names of the Font Awesome icon displayed in the button (for instance "fas fa-air-freshener").',
                ],
                'title' => [
                    'type' => ['string', 'null'],
                    'description' => 'The title displayed over the image.',
                ],
                'titleFormat' => [
                    'type' => 'string',
                ...
                'body' => $schemaFactory->describeReference(ExportDeclarations::REFERENCE_CONTENT, [
                    'type' => ['string', 'null'],
                    'description' => 'The rich text displayed below the title, over the image.',
                ]),
                'buttonText' => [
                    'type' => ['string', 'null'],
