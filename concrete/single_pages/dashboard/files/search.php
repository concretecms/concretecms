    ...
    <div id="ccm-search-results-table">
        <table class="ccm-search-results-table" data-search-results="files">
            ...
            foreach ($result->getItems() as $item) { ?>
                <tr data-details-url="javascript:void(0)" data-item-type="<?=$item->getItemType()?>" data-item-id="<?=$item->getItem()->getTreeNodeID()?>">
        ...
        (function ($) {
            $(function () {
                $('table[data-search-results=files]').concreteFileManagerTable({
                    ...
                    'highlightFolders': <?=json_encode($highlightFolders)?>
                });
            });
        })(jQuery);
    </script>
