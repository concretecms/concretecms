            ...
            if ($p->getPackageID() > 0) {
                ...
                $packageName = t('None');
					...
					<tr>
						...
						<td style="width: 30%"><?php echo $packageName; ?></td>
						<td style="width: 1">
							<?php if ($cp->canAdmin()) {
                ?>
                                <a href="<?=$view->action('refresh', $p->getCollectionID(), $this->controller->token->generate('refresh')); ?>" title="<?=t('Refreshes the page, rebuilding its permissions and its name.'); ?>" class="icon-link launch-tooltip"><i class="fas fa-sync"></i></a>
                            <?php
            } ?>
						</td>
					</tr>
