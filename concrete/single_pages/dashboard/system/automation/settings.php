...
<form method="post" action="<?= $view->action('submit') ?>">
    <div class="form-group">
        ...
        <div class="form-check">
            <?= $form->radio('listening', 'automatic', $listening === 'automatic') ?>
            <label class="form-check-label" for="listening1"><?= t(
                    'Automatic - Works without advanced configuration.'
                ) ?></label>
        </div>
        <div class="form-check">
            <?= $form->radio('listening', 'manual', $listening === 'manual') ?>
    ...
    <div class="alert alert-warning" data-notice="listener" style="display: none"><?= t(
            'If you enable manual listening you <b>must</b> ensure the queue listener is running at least one worker: <code>concrete/bin/concrete5 queue:process</code>'
        ); ?></div>
