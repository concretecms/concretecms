...
abstract class Workflow extends ConcreteObject implements \Concrete\Core\Permission\ObjectInterface
{
    ...
    {
        $db = Loader::db();
        $db->Execute('delete from Workflows where wfID = ?', [$this->wfID]);
    ...
    {
        $db = Loader::db();
        $wfID = $db->getOne('SELECT wfID FROM Workflows WHERE wfName=?', [$name]);
        if (!$wfID) {
            ...
            $db->Execute('insert into Workflows (wftID, wfName, pkgID) values (?, ?, ?)', [$wt->getWorkflowTypeID(), $name, $pkgID]);
            $wfID = $db->Insert_ID();
        }
    ...
    {
        $db = Loader::db();
        $db->Execute('update Workflows set wfName = ? where wfID = ?', [$wfName, $this->wfID]);
    }
    ...
    {
        $db = Loader::db();
        $r = $db->GetRow('select Workflows.* from Workflows where Workflows.wfID = ?', [$wfID]);
