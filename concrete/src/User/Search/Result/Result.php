...
class Result extends SearchResult
{
    public function getItems()
    {
        if (!isset($this->items)) {
            ...
            $includeIndirectMemberships = false;
            foreach ($this->listColumns->getColumns() as $column) {
                if ($column instanceof DirectMembershipsCountColumn || $column instanceof DirectMembershipsColumn) {
                    $includeMemberships = true;
                } elseif ($column instanceof IndirectMembershipsColumn) {
                    $includeMemberships = true;
                    $includeIndirectMemberships = true;
                }
            }
            if ($includeMemberships) {
                ...
                $membershipsProvider->preload($userIDs, $includeIndirectMemberships);
            }
