...
use Concrete\Theme\OxfordShirt\PageTheme;
use Doctrine\ORM\EntityManager;

class OxfordShirtDocumentationProvider implements DocumentationProviderInterface
{
    ...
    public function clearSupportingElements(): void
    {
        ...
        $expressObject = $em->getRepository(Entity::class)->findOneByHandle('oxford_shirt_employee');
        if ($expressObject) {
        ...
        $documentation = FileFolder::getNodeByName('Oxford Shirt Documentation');
        if ($documentation) {
        ...
        $calendar = $em->getRepository(CalendarEntity::class)->findOneByName('Oxford Shirt Sample Calendar');
        if ($calendar) {
            $em->remove($calendar);
            $em->flush();
        }
    ...
    public function installSupportingElements(): void
    {
        ...
        $filesystem->addFolder($root, 'Oxford Shirt Documentation');
        ...
        // We need this because if you haven't installed Oxford Shirt Full (i.e. you've installed Elemental instead)
        ...
        $object = $express->getObjectByHandle('oxford_shirt_employee');
        $express->refresh($object);

        for ($i = 0; $i < count($employeeData); $i++) {
            $employeeEntryData = $employeeData[$i];
            $express->buildEntry('oxford_shirt_employee')
        ...
        $calendar->setName('Oxford Shirt Sample Calendar');
        $em->persist($calendar);
        $em->flush();
