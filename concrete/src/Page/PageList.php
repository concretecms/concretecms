...
class PageList extends DatabaseItemList implements PagerProviderInterface, PaginationProviderInterface
{
    ...
    public function finalizeQuery(\Doctrine\DBAL\Query\QueryBuilder $query)
    {
        ...
        switch ($this->pageVersionToRetrieve) {
            ...
            default:
                $now = new \DateTime();
                $query->andWhere('cvID = (select cvID from CollectionVersions where cID = cv.cID and cvIsApproved = 1 and (cvPublishDate <= :cvPublishDate or cvPublishDate is null) order by cvPublishDate desc limit 1)');
