...
class RuleResolver
{
     ...
     * In order to view/edit them, open https://mlocati.github.io/php-cs-fixer-configurator/#version:3.1|configurator and paste there this array.
     *
     * @var array
     */
    protected const INVARIANT_RULES = [
        ...
        'general_phpdoc_tag_rename' => [
            'replacements' => ['inheritdocs' => 'inheritdoc', 'inheritdoc' => 'inheritdoc'],
        ],
        ...
        // Works for class constants, properties and methods only.
        ...
        // Inside class or interface element self `self` should be preferred to the class name itself.
        'self_accessor' => true,
     ...
     */
    public function describeFlags(int $flags): string
    {
        $flags = (int) $flags;
        if (($flags & PhpFixer::FLAG_PSR4CLASS) === PhpFixer::FLAG_PSR4CLASS) {
            return 'PHP-only files with the same name as the class it implements';
        }
        if (($flags & PhpFixer::FLAG_BOOTSTRAP) === PhpFixer::FLAG_BOOTSTRAP) {
            return 'PHP-only files with syntax compatible with old PHP versions';
        }
        if ($flags === PhpFixer::FLAG_NONE) {
            return 'files with mixed contents';
        }

        $chunks = [];
        if (($flags & PhpFixer::FLAG_OLDPHP) === PhpFixer::FLAG_OLDPHP) {
            $chunks[] = 'files with syntax compatible with old PHP versions';
        }
        if (($flags & PhpFixer::FLAG_PHPONLY) === PhpFixer::FLAG_PHPONLY) {
            $chunks[] = 'PHP-only files';
        }
     ...
     */
    public function getRules(int $flags, bool $onlyAvailableOnes, string $minimumPhpVersion = ''): array
    {
        $hasFlag = static function(int $flag) use ($flags): bool {
            return ($flags & $flag) === $flag;
        };
        $atLeastVersion = static function(string $wantedPhpVersion, bool $phpVersionMustBeCurrent = false) use ($hasFlag, $minimumPhpVersion): bool {
            if ($hasFlag(PhpFixer::FLAG_OLDPHP)) {
                return false;
            }
            if (version_compare($minimumPhpVersion, $wantedPhpVersion) < 0) {
                return false;
            }
            if ($phpVersionMustBeCurrent && version_compare(PHP_VERSION, $wantedPhpVersion) < 0) {
                return false;
            }
        ...
        $rules = [
            ...
            // @see \Concrete\Core\Support\CodingStyle\PhpFixer\Fixer\InlineTagFixer
            'ConcreteCMS/inline_tag' => true,

            // PHP arrays should be declared using the configured syntax.
            'array_syntax' => [
                'syntax' => $atLeastVersion('5.4') ? 'short' : 'long',
            ],
            ...
            'braces' => $hasFlag(PhpFixer::FLAG_PHPONLY)
                ? [
            ...
            // Requires PHP >= 7.0.
            'combine_nested_dirname' => $atLeastVersion('7.0') ? true : false,

            // Replaces `dirname(__FILE__)` expression with equivalent `__DIR__` constant.
            'dir_constant' => $atLeastVersion('5.3') ? true : false,

            // Replaces short-echo `<?=` with long format `<?php echo`/`<?php print` syntax, or vice-versa.
            'echo_tag_syntax' => [
                'format' => $atLeastVersion('5.4') ? 'short' : 'long',
            ],

            // Add curly braces to indirect variables to make them clear to understand.
            // Requires PHP >= 7.0.
            'explicit_indirect_variable' => $atLeastVersion('7.0') ? true : false,

            // Replace core functions calls returning constants with the constants.
            'function_to_constant' => $atLeastVersion('5.5')
                ? true
                : [
            ...
            // Requires PHP >= 7.3.
            'heredoc_indentation' => $atLeastVersion('7.3')
                ? [
            ...
            // Requires PHP >= 7.1.
            'list_syntax' => [
                'syntax' => $atLeastVersion('7.1') ? 'short' : 'long',
            ],
            ...
            'nullable_type_declaration_for_default_null_value' => [
                'use_nullable_type_declaration' => $atLeastVersion('7.1') ? true : false,
            ],

            // `@return void` and `@return null` annotations should be omitted from PHPDoc.
            'phpdoc_no_empty_return' => $atLeastVersion('7.0') ? true : false,

            // Converts `pow` to the `**` operator.
            'pow_to_exponentiation' => $atLeastVersion('5.6') ? true : false,

            // Classes must be in a path that matches their namespace, be at least one namespace deep and the class name should match the file name.
            'psr_autoloading' => $hasFlag(PhpFixer::FLAG_PSR4CLASS) ? true : false,

            // Callables must be called without using `call_user_func*` when possible.
            'regular_callable_call' => $atLeastVersion('5.6') ? true : false,

            // Instructions must be terminated with a semicolon.
            'semicolon_after_instruction' => $hasFlag(PhpFixer::FLAG_PHPONLY) ? true : false,

            // Use the Elvis operator `?:` where possible.
            'ternary_to_elvis_operator' => $atLeastVersion('5.3') ? true : false,

            // Use `null` coalescing operator `??` where possible.
            // Requires PHP >= 7.0.
            'ternary_to_null_coalescing' => $atLeastVersion('7.0') ? true : false,

            // Multi-line arrays, arguments list and parameters list must have a trailing comma.
            'trailing_comma_in_multiline' => [
                'elements' => $atLeastVersion('8.0', true)
                ...
                'after_heredoc' => $atLeastVersion('7.3') ? true : false,
            ],

            'use_arrow_functions' => $atLeastVersion('7.4') ? true : false,

            // Visibility MUST be declared on all properties and methods; `abstract` and `final` MUST be declared before the visibility; `static` MUST be declared after the visibility.
            'visibility_required' => [
                'elements' => $atLeastVersion('7.1')
     ...
     */
    public function getFixers(array $rules): array
    {
    ...
    protected function getFixerFactory(): FixerFactory
    {
        if ($this->fixerFactory === null) {
